#!/usr/bin/env python3
"""Recipe for training a continuous hifi-gan vocoder on custom Whisper-based features using LJSpeech dataset.
...
Authors
 * Modified for LJSpeech dataset with custom Whisper-based continuous features
"""
...
class HifiGanBrain(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        x, _ = batch.custom_features  # continuous features from custom encoder
        y, _ = batch.sig
    ...
    def run_inference_sample(self, name, epoch):
        ...
        with torch.no_grad():
            if self.last_batch is None:
                return
            x, y = self.last_batch  # No speaker embedding
            ...
            # For continuous features, no need to apply unique_consecutive
...
def dataio_prepare(hparams):
    ...
    custom_hop_size = hparams["custom_hop_size"]
    custom_features_folder = pl.Path(hparams["custom_features_folder"])

    # Define audio pipeline for continuous features (no speaker embeddings):
    @sb.utils.data_pipeline.takes("id", "wav", "segment")
    @sb.utils.data_pipeline.provides("custom_features", "sig")
    def audio_pipeline(utt_id, wav, segment):
        info = torchaudio.info(wav)
        audio = sb.dataio.dataio.read_audio(wav)
        
        # Resample to 16kHz if necessary (LJSpeech is 22050Hz, Whisper needs 16kHz)
        ...
        # Load continuous custom features with shape [T, 1, D]
        custom_features = np.load(custom_features_folder / f"{utt_id}.npy")
        custom_features = torch.FloatTensor(custom_features)  # [T, 1, D]

        # Trim end of audio to match custom features
        feature_length = min(audio.shape[0] // custom_hop_size, custom_features.shape[0])
        custom_features = custom_features[:feature_length]  # [T', 1, D]
        audio = audio[: feature_length * custom_hop_size]

        # Ensure minimum length by repeating if necessary
        while audio.shape[0] < segment_size:
            audio = torch.hstack([audio, audio])
            custom_features = torch.vstack([custom_features, custom_features])
        
        audio = audio.unsqueeze(0)  # [1, audio_length]

        if segment:
            # For continuous features with shape [T, 1, D], we need to handle the time dimension
            # Transpose to [1, D, T] for sampling, then back to [T, 1, D]
            custom_features = custom_features.permute(1, 2, 0)  # [1, D, T]
            audio, custom_features = sample_interval([audio, custom_features], segment_size)
            custom_features = custom_features.permute(2, 0, 1)  # [T, 1, D]

        return custom_features, audio
    ...
    for dataset in hparams["splits"]:
        datasets[dataset] = sb.dataio.dataset.DynamicItemDataset.from_json(
            ...
            dynamic_items=[audio_pipeline],  # Removed speaker pipeline
            output_keys=["id", "custom_features", "sig"],  # Removed spk_emb
        )
...
if __name__ == "__main__":
    ...
    # LJSpeech data preparation (modified from THCHS30)
    ...
    sb.utils.distributed.run_on_main(
        ...
        kwargs={
            ...
            "train_ratio": hparams.get("train_ratio", 0.8),
            "valid_ratio": hparams.get("valid_ratio", 0.1),
            "test_ratio": hparams.get("test_ratio", 0.1),
            "seed": hparams["seed"],
            "skip_prep": hparams["skip_prep"],
        },
    )

    # Feature extraction (reuse existing extract_code function)
    from extract_code import extract_ljspeech_custom

    sb.utils.distributed.run_on_main(
        extract_ljspeech_custom,
        kwargs={
            "data_folder": hparams["save_folder"],  # JSON files location
            "splits": hparams["splits"],
            "whisper_model_name": hparams["whisper_model_name"],
            "feature_extractor_config": hparams["feature_extractor"],
            "encoder_config": hparams["encoder"],
            "save_folder": hparams["save_folder"],  # Features saved under save_folder/custom_features
            "sample_rate": hparams["sample_rate"],
            "skip_extract": hparams["skip_extract"],
            "freeze_encoder": hparams["freeze_encoder"],
            "layer_id": hparams["layer_id"],
        },
    )
    ...
    continuous_hifi_gan_brain = HifiGanBrain(
