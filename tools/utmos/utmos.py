# Copyright (c) 2024 Mingqi Jiang (mqjiang_learning@163.com)
#
...
code copy from https://github.com/gemelo-ai/vocos/blob/main/metrics/UTMOS.py
"""
...
class UTMOSScore:
    ...
    def __init__(self, device, ckpt_path="epoch=3-step=7459.ckpt"):
        ...
        # PyTorch 2.6 默认 weights_only=True 会阻止包含 OmegaConf 的 checkpoint 反序列化
        # UTMOS 官方 ckpt 需要完整加载，因此显式关闭 weights_only
        # 允许加载包含 OmegaConf、fairseq meters 等对象的旧版 Lightning ckpt
        torch.serialization.add_safe_globals([argparse.Namespace, fairseq.meters.AverageMeter])
...
def load_ssl_model(ckpt_path="wav2vec_small.pt"):
    ...
    SSL_OUT_DIM = 768
    # 允许特定类型反序列化（PyTorch 2.6 默认 weights_only=True）
    torch.serialization.add_safe_globals([argparse.Namespace, fairseq.meters.AverageMeter])
