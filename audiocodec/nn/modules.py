    ...
    def forward(self, input_features, input_length, output_hidden_states=False):
        ...
        if output_hidden_states:
            ...
            for layer_hidden_states in hidden_states_all_layers:
                ...
                # Transpose to [bsz, d_model, tgt_len] format
                masked_layer_states = masked_layer_states.transpose(1, 2)  # [bsz, d_model, tgt_len]
...
class OmniAudioDecoder(nn.Module):
    ...
    def forward(self, hidden_states, input_length): # (B, D, T)
        ...
        output_features = self.deconv1(hidden_states) # (B, D, T)
        output_features = self.deconv2(output_features) # (B, D, T)
