...
class NormConv2d(nn.Module):
    """
    带归一化的2D卷积层
    简化版实现，支持weight_norm和spectral_norm
    """
    ...
    def __init__(self, *args, norm: str = 'none', norm_kwargs: tp.Dict[str, tp.Any] = {}, **kwargs):
        ...
        # 应用后处理归一化
        if norm == 'layer_norm':
            self.norm = nn.LayerNorm(self.conv.out_channels, **norm_kwargs)
        elif norm == 'batch_norm':
            self.norm = nn.BatchNorm2d(self.conv.out_channels, **norm_kwargs)
        elif norm == 'group_norm':
            num_groups = norm_kwargs.get('num_groups', 1)
            self.norm = nn.GroupNorm(num_groups, self.conv.out_channels, **norm_kwargs)
        else:
